...
class PixelEvent {
    ...
    private function validateAndEnrich($data) {
        // Buscar configuração do pixel pelo pixel_id (hash)
        $pixelConfig = null;
        if (isset($data['pixel_id'])) {
            $pixelConfig = $this->getPixelConfig($data['pixel_id']);
        }
        ...
        $enriched = [
            ...
            'user_id' => $pixelConfig['user_id'] ?? $data['user_id'] ?? null,
            'integration_id' => $pixelConfig['integration_id'] ?? $data['integration_id'] ?? null,
    ...
    private function getPixelConfig($pixelHash) {
        $sql = "SELECT user_id, integration_id, status
                FROM pixel_configurations
                WHERE pixel_hash = :pixel_hash
                AND status = 'active'
                LIMIT 1";

        $stmt = $this->conn->prepare($sql);
        $stmt->bindParam(':pixel_hash', $pixelHash);
        $stmt->execute();
