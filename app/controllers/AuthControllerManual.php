<?php
/**
 * Controller de Autenticação - Versão Manual OAuth
 * Não depende do vendor/ - funciona sem Composer
 */

require_once '../config/database.php';
require_once '../oauth_manual.php';

class AuthControllerManual {
    private $db;
    private $user;
    ...
    public function googleLogin() {
        if (empty(GOOGLE_CLIENT_ID) || GOOGLE_CLIENT_ID === 'teste-google-id') {
            $this->redirectWithError('/login', 'Credenciais Google não configuradas. Configure as credenciais reais em config/app.php');
            return;
        }

        $provider = new SimpleGoogleOAuth(
            GOOGLE_CLIENT_ID,
        ...
        if (!isset($_GET['code'])) {
            $authUrl = $provider->getAuthUrl(['openid', 'profile', 'email']);
            ...
            try {
                $tokenData = $provider->getAccessToken($_GET['code']);
                
                if (isset($tokenData['access_token'])) {
                    $userData = $provider->getUserInfo($tokenData['access_token']);
                    $this->processOAuthUser('google', $userData);
                } else {
                    $this->redirectWithError('/login', 'Erro ao obter token do Google.');
                }
            } catch (Exception $e) {
                $this->redirectWithError('/login', 'Erro na autenticação Google: ' . $e->getMessage());
            }
        }
    }
    ...
    private function processOAuthUser($provider, $userData) {
        ...
        if (!$existingUser && method_exists($this->user, 'findByOAuthProvider')) {
            $existingUser = $this->user->findByOAuthProvider($provider, $providerId);
        }

        if ($existingUser) {
            if (method_exists($this->user, 'updateOAuthData')) {
