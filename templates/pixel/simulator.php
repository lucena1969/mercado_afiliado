...
fetch('<?= BASE_URL ?>/api/pixel/collect', {
    method: 'POST',
    headers: {
    ...
    body: JSON.stringify(${JSON.stringify(eventData, null, 2)})
        ...
        async function sendTestEvent() {
            ...
            try {
                const response = await fetch('<?= BASE_URL ?>/api/pixel/collect', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json'
                    },
            ...
            let resultsHTML = `
                <div class="result-item">
                    <div class="result-header">
                        ...
                        <span class="result-status ${result.success ? 'result-success' : 'result-error'}">
                    ...
                    ${result.success ?
                        '<p style="color: #059669;">Evento enviado com sucesso para todas as plataformas configuradas!</p>' :
                        `<p style="color: #dc2626;">Erro: ${result.error || 'Erro desconhecido'}</p>`
                    }
                </div>
            `;
